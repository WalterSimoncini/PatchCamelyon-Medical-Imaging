...
def main(args):
    ...
    out_dataset = DatasetDict({
        "train": create_dataset(data_dir=args.data_dir, split="train", num_proc=args.num_proc),
        "valid": create_dataset(data_dir=args.data_dir, split="valid", num_proc=args.num_proc),
        "test": create_dataset(data_dir=args.data_dir, split="test", num_proc=args.num_proc)
    })
...
def create_dataset(data_dir: str, split: str, num_proc: int) -> Dataset:
...
def get_data_arrays(data_dir: str, split: str) -> Tuple[np.ndarray, np.ndarray]:
    """
